...
## [9.1.0] — 2026-09-21 · 解析与交互修复 + 移动端与工具栏升级

### 修复
- `FX_LIB.parseTimeExpr` 入口前置 `U.normChars` 归一化：全角字母/括号/数字、Unicode 负号、上标、`**` 幂写法不再被 mathjs 判非法（交互求解「无法解析」）；explore 传函/时域识别改用归一化文本，全角 `ｓ／` 同样可判
- `DSP.ifft` 补共轭还原（变换后 `im = -im/n`），修正虚部符号
- 复平面/输入组件资源泄漏：`pid`/`explore` 的 `ComplexPlane` 与 `system`/`zt` 的 MI 防抖定时器在模块切换时未释放，补齐 `dispose()`/`destroy()`
- 移除 `tests/module-pz.test.mjs` 遗留调试输出

### 新增
- 系统框图连线避障：`routePath` 生成 orig/下绕/上绕三候选并按穿越元件数择优，同轴穿框改绕行；无法完全避障时降透明度并提示一次
- 系统框图反向拉线：可从任意端口（含入端口）拖出连线，方向自动判定（源→目标），去重/禁自连对双向一致
- 结果工具栏移动端收纳：窄屏（≤480px）主操作直显、次级收进「⋯ 更多」底部 action-sheet（命中区 ≥44px），行为与文案不变

### 优化
- 移动端 ≤480px：交互控件命中区统一 ≥44px、工作台卡片单列、符号键盘横向滚动防遮挡
- 版本 9.1.0 / ?v=9.1 / sw VER fltp-9.1；补 blk-interaction（避障绕行/双击编辑/反向拉线方向）、safeeval（归一化）回归断言

## [9.0.0] — 2026-09-19 · 拉普拉斯多 ROC 分析

### 新增
- `TR.rocs`：按极点实部划分 F(s) 的全部收敛域，逐域判定
  因果（最右）/反因果（最左）/双边与稳定性（ROC 含虚轴 ⇔ 双边稳定）
- 拉普拉斯模块「ROC 分析」面板：极点列表 + 全部 ROC 表 + 稳定域结论
  （jω 轴极点 → 明确提示不存在含虚轴的 ROC）
- 浏览器实测 1/((s+1)(s−2))：三区域、中间域双边稳定
- 版本 9.0.0 / ?v=9.0 / sw VER fltp-9.0
...
### 新增
- explore「方程求解 · 数据序列」模式新增运算选择：序列分析 / **卷积 x*h** /
  **互相关 x⋆h**（第二序列 h(n) 输入，DSP.conv 线性卷积，互相关=卷积翻转）
- 输出统计：运算类型、长度（n⋅m→n+m−1）、能量 Σy²；结果含频谱
- 版本 8.8.0 / ?v=8.8 / sw 同步（pwa 契约测试捕获过一次漏改并已修正流程）

## [8.7.0] — 2026-09-19 · 根轨迹 Worker 化

### 新增
- `DSP.rootLocus`：根轨迹纯函数内核（自 system.js 提取，含分支追踪/
  渐近线/分离点/临界增益），可被 Worker 与测试直接调用
- 系统分析根轨迹迁移至 Worker 池：主线程零阻塞，计算期间显示
  「根轨迹计算中…」，完成后自动重绘
- `tests/rootlocus.test.mjs` 11 项（经典案例 Ku=6/质心/分离点交叉验证）
- 版本 8.7.0 / ?v=8.7
...
### 新增
- 傅立叶变换「时域/频域」页签新增能量守恒指标：时域能量 ∫|x|²dt、
  频域能量 ∫|X|²df（FFT 计算）与 Parseval 比值（DFT 域精确成立，比值=1.000000）
- 浏览器实测矩形/高斯等多信号比值均为 1；版本 8.4.0 / ?v=8.4

## [8.3.0] — 2026-09-19 · 拉普拉斯性质数值验证器

### 新增
- `TR.propVerify`：时移/频移/尺度/微分四条性质的数值验证——在 Re(s)>ROC
  实轴点上比较「变换后信号的定义积分」与「符号公式」（FsNumeric 闭式），
  相对误差 &lt; 1e-5 判定通过；积分起点对齐间断点、微分边界单侧差分
- 拉普拉斯模块「性质数值验证器」面板：信号输入（MI）+ 性质选择 +
  验证表（定义积分/公式值/相对误差）+ 公式 KaTeX
- `tests/lapprops.test.mjs` 12 项（四性质 × 指数/正弦/组合信号）
- 版本 8.3.0 / ?v=8.3
